...
See Figure S1 and S2 for ROSES diagram. BMP data from the systematic review is available in @kikoyoTWRINPSBMPDB2024.
...
where *v* represents the sampling variance, $CV_{control,i}$ and $CV^2_{experiment,i}$ are the coefficients of variation from the *i*th study for studies 1, 2, ..., *K*. *n~control~* and *n~experiment~* are the number of samples in the control (pre-treatment) trial or experimental (post-treatment) trial respectively.
...
We did identify a wide variety of BMPs in the review, but it did not appear that any particular type of BMP was responsible for the majority of studies for any given parameter (Figure \@ref(fig:fig1) C).
Our review was restricted to studies published after between 1999 and 2023.
The number of studies published for each parameter were roughly uniformly distributed over time (Figure \@ref(fig:fig2) A) and are not indicative of increases or decreases in the number of published studies.
Study length was strongly skewed for all parameters (Figure \@ref(fig:fig2) B). Median study lengths were 3 (DIN), 2 (FIB), 2.5 (PO~4), 2.5 (TN), 2.5 (TP), and 2 (TSS) years. 
There appears to be a strong clustering of BMP studies in the mid-Atlantic region (North Carolina, Virginia, Maryland) with other states sparsely represented or completely absent from the review (Figure \@ref(fig:fig3)).
...
The overall mean effect (estimated with the intercept only multilevel random effects model) showed significant mean reductions in FIB (ROM = 0.85, 95% CI [0.36, 1.34]; BMP~eff~ = 57.4%, 95% CI [30.4%, 73.9%]; Figure \@ref(fig:fig4)) resulting from BMPs. 
...
Increased influent concentrations ($\beta$ = 0.25, 95% CI [0.14, 0.37]) resulted in significantly larger ROM effect for FIB (Figure \@ref(fig:fig5)).
...
Overall effects showed that BMPs resulted in significant mean reductions in TN (ROM = 0.42, 95% CI [0.21, 0.62]; BMP~eff~ = 34.0%, 95% CI [18.7%, 46.4%]; Figure \@ref(fig:fig4)) but not in DIN (ROM = 0.64, 95% CI [-0.08, 1.35]; BMP~eff~ = 47.1%, 95% CI [-8.1%, 74.1%]; Figure \@ref(fig:fig4)).
...
Influent concentration ($\beta$ = 0.23, 95% CI [-0.035, 0.49]) was not significant at the 95% confidence level for the TP model (Figure \@ref(fig:fig6); Table S4).
Influent concentration ($\beta$ = 0.27, 95% CI [0.085, 0.44]) was significant for the PO~4~ model (Figure \@ref(fig:fig6); Table S5).
...
Our systematic review revealed strong spatial disparities in published BMP studies (Figure \@ref(fig:fig3)).
...
Similarly, we found detention type BMPs clustered only in humid climates (high aridity index) which reduces our confidence in extrapolating the interaction between BMP types and aridity index for FIB (Figure \@ref(fig:fig5)).
Not only were there spatially disparities, but we observed that the relative distribution of aridity index values does not resemble the distribution of aridity values across the U.S. (Figure \@ref(fig:fig7)).
...
Slopes are a valid effect size for use in meta-analysis but the set of covariates used between studies should be the same since the coefficient of interest is adjusted to account for other terms in the the regression model [@beckerSynthesisRegressionSlopes2007].
...
knitr::include_graphics("frontiers_submission_files/figure-latex/fig1.pdf")
...
knitr::include_graphics("frontiers_submission_files/figure-latex/fig2.pdf")
...
knitr::include_graphics("frontiers_submission_files/figure-latex/fig3.pdf")
...
knitr::include_graphics("frontiers_submission_files/figure-latex/fig4.pdf")
```
...
knitr::include_graphics("frontiers_submission_files/figure-latex/fig5.pdf")
```
...
knitr::include_graphics("frontiers_submission_files/figure-latex/fig6.pdf")
```
...
knitr::include_graphics("frontiers_submission_files/figure-latex/fig7.pdf")
```

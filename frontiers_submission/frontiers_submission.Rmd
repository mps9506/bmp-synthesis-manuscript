...
The systematic review followed guidance provided in the Collaboration for Environmental Evidence systematic review guidelines [@collaborationforenvironmentalevidenceGuidelinesStandardsEvidence2018; Figures S1-S2].
...
Slopes are a valid effect size for use in meta-analysis but the set of covairates used between studies should be the same since the coeffecient of interest is adjusted to account for other terms in the the regression model [@beckerSynthesisRegressionSlopes2007].
...
The FIB results are useful in particular because the general consensus has been that the effectiveness of BMPs at reducing FIBs and pathogens has been sparsely reviewed and understudied [@hagerPerformanceLowimpactDevelopment2019].
...
The effect of higher FIB removal rates at high concentrations is expected as filtration and other removal processes associated with BMPs become saturated(?)... something about sediment associated fib...
...
While our models capture some of the variance due to these differences as *between study* effects, including these as moderators in a meta-regression model would be valuable but these details are under reported in BMP studies.
...
This is result is largely consistent with findings in reviews by @kochNitrogenRemovalStormwater2014, @hagerPerformanceLowimpactDevelopment2019 and @horvathEffectsRegionalClimate2023.
...
For example, @valencaNitrateRemovalUncertainty2021, using data from the International Stormwater Database, showed that the relative importance of climate and design variables for moderating nitrogen removal varied by BMP type.





*compare nutrient effects to koch, barrett, clary*
...
*data exploration did not suggest this was the case, but it is reasonable to suggest the the underlying relationship between ROM and any of the moderators are a nonlinear function. Semi-parametric approaches might provide some additional  insight*
